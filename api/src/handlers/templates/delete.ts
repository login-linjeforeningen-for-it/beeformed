...
const selectOwnerSql = await loadSQL('templates/selectOwnerForUpdate.sql')
const deleteSql = await loadSQL('templates/delete.sql')
...
export default async function deleteTemplate(req: AuthenticatedRequest<{ Params: IdParams }>, res: FastifyReply) {
    await runInTransaction(async (client) => {
        ...
        if (ownerResult.rows.length === 0) throw new HttpError(404, 'Template not found')
